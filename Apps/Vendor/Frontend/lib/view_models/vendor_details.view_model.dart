...
import 'widgets/bottomsheets/payout.bottomsheet.dart';
...
class VendorDetailsViewModel extends MyBaseViewModel {
  ...
  fetchVendorDetails() async {
    ...
    try {
      ...
      // The response is a Vendor object
      vendor = response;
      totalEarning = 0.0; // Will be populated from stats API
      totalOrders = 0; // Will be populated from stats API
      weeklySales = []; // Will be populated from stats API
      notifyListeners();
      clearErrors();
    } catch (error) {
      print("Error ==> $error");
