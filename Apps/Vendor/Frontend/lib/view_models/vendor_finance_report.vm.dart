...
import 'dart:io';
import 'models/report/earning_data_source.dart';
import 'models/report/sale_data_source.dart';
import 'models/report/sale_report.dart';
import 'services/alert.service.dart';
import 'services/auth.service.dart';
import 'view_models/vendor_details.view_model.dart';
import 'services/report_pdf.service.dart';
...
//
  ...
  fetchSalesReport([bool loadMore = false]) async {
    ...
    try {
      ...
      if (loadMore) {
        // Handle Firebase response - salesReport is a Map, extract data array
        if (salesReport is Map && salesReport.containsKey('data')) {
          final data = salesReport['data'] as List;
          salesReportDataset.addAll(data.map((item) => SaleReport.fromJson(item)).toList());
        }
      } else {
        // Handle Firebase response - salesReport is a Map, extract data array
        if (salesReport is Map && salesReport.containsKey('data')) {
          final data = salesReport['data'] as List;
          salesReportDataset = data.map((item) => SaleReport.fromJson(item)).toList();
        }
      }
  ...
  fetchEarningsReport([bool loadMore = false]) async {
    ...
    try {
      ...
      if (loadMore) {
        // Handle Firebase response - earningsReport is a Map, extract data array
        if (earningsReport is Map && earningsReport.containsKey('data')) {
          final data = earningsReport['data'] as List;
          earningsReportDataset.addAll(data.cast<Map<String, dynamic>>());
        }
      } else {
        // Handle Firebase response - earningsReport is a Map, extract data array
        if (earningsReport is Map && earningsReport.containsKey('data')) {
          final data = earningsReport['data'] as List;
          earningsReportDataset = data.cast<Map<String, dynamic>>();
        }
      }
  ...
  exportSalesReport(BuildContext context) async {
    ...
    try {
      ...
      String? path;
      String fileLocation = "";
      // For web compatibility, use application support directory
