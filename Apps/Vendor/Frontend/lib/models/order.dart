...
import 'order_fee.dart';
import 'order_service.dart';
import 'order_status.dart';
import 'order_stop.dart';
...
class Order {
  ...
  String get deliveryInstructions => deliveryAddress?.address ?? '';
  List<Map<String, dynamic>> get items => orderProducts?.map((p) => {
    'id': p.id,
    'name': p.product?.name ?? '',
    'quantity': p.quantity,
    'price': p.price,
  ...
  double get totalAmount => total ?? 0.0;
  ...
  Order copyWith({
    ...
    return Order(
      ...
      rateDriver: rateDriver ?? this.rateDriver,
      code: code ?? this.code,
      ...
      orderStops: orderStops ?? this.orderStops,
      user: user ?? this.user,
